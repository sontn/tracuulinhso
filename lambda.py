...
def tam_mot_linh_so_func(phone_number):
    ...
    if not str(phone_number)[-4:].isdigit():
        print("Số điện thoại không hợp lệ")
        exit()

    last_four_digits = int(str(phone_number)[-4:])
    ...
    # print(f"Bốn số cuối: {last_four_digits}")   

    # Check xem linh_so có trong tam_mot_linh_so không
    if linh_so not in tam_mot_linh_so:
        return (f"Khóa {linh_so} không tồn tại trong 81 linh số")        
